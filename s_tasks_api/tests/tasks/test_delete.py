...
from .utils import BaseTaskTestCase, get_detail_task_url, get_detail_group_task_url, get_remove_to_my_task_url
...
class DeleteGroupTaskTestCase(BaseTaskTestCase):
    ...
    def test_remove_to_my_task___not_created_user___403(self):
        # Arrange
        other_member_task = GroupTask.objects.filter(task__created_by=self.member_2.pk).first()
        # Act
        response = self.client.delete(get_remove_to_my_task_url(other_member_task.pk))
        # Assert
        self.assertEqual(status.HTTP_403_FORBIDDEN, response.status_code)
    ...
    def test_remove_to_my_task___task_is_created_by_member1___203(self):
        # Arrange
        group_task = GroupTask.objects.filter(task__created_by=self.member_1.pk).first()
        task = group_task.task
        # Act
        response = self.client.delete(get_remove_to_my_task_url(group_task.pk))
        # Assert
        self.assertEqual(status.HTTP_204_NO_CONTENT, response.status_code)
        self.assertFalse(GroupTask.objects.filter(pk=group_task.pk).exists())
